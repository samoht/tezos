...
class TestBakerMultisig:
    ...
    def test_transfer(self, client: Client):
        prep_transfer = [
            'prepare', 'baker', 'transaction', 'on', BAKER,
            'transferring', '10', 'to', 'bootstrap1']
        client.run(prep_transfer)
        sign_transfer = [
            'sign', 'baker', 'transaction', 'on', BAKER,
            'transferring', '10', 'to', 'bootstrap1',
            'with', 'key']
        sig0 = client.run(sign_transfer + [KEY0])[:-1]
        sig1 = client.run(sign_transfer + [KEY1])[:-1]
        call_transfer = [
            'from', 'multisig', 'baker', BAKER,
            'transfer', '10', 'to', 'bootstrap1',
            'on', 'behalf', 'of', SRC, 'with', 'signatures', sig0, sig1]
    ...
    def test_submit_proposals(self, client: Client):
        prep_proposal = [
            'prepare', 'baker', 'transaction', 'on', BAKER,
            'submitting', 'proposals', PROTO_A, PROTO_B]
    ...
    def test_submit_ballot(self, client: Client):
        ...
        utils.bake_until_cycle_end(client, 'baker5', BAKE_ARGS,
                                   blocks_per_cycle=BLOCKS_PER_CYCLE)

        prep_ballot = [
            'prepare', 'baker', 'transaction', 'on', BAKER,
            'submitting', 'ballot', PROTO_A, '100', '0', '0']
    ...
    def test_set_declining_delegation(self, client: Client):
        prep_decline_delegations = [
            'prepare', 'baker', 'transaction', 'on', BAKER,
            'setting', 'it', 'declining', 'new', 'delegations']
        ...
        call_decline_delegations = [
            'set', 'multisig', 'baker', BAKER,
            'declining', 'new', 'delegations',
            'on', 'behalf', 'of', SRC, 'with', 'signatures', sig0, sig1, sig2]
    ...
    def test_set_consensus_key(self, client: Client):
        ...
        prep_set_consensus_key = [
            'prepare', 'baker', 'transaction', 'on', BAKER,
            'setting', 'consensus', 'key', 'to', new_key]
    ...
    def test_generic_call(self, client: Client):
        ...
        call_generic_call = [
            'call', 'generic', 'multisig', 'baker', BAKER,
            'with', 'lambda', param,
            'on', 'behalf', 'of', SRC, 'with', 'signatures', sig0, sig1]
    ...
    def test_set_owner_keys(self, client: Client):
        ...
        prep_set_owner_keys = [
            ...
            'and', 'owner', 'keys', 'to', new_key0, new_key1]
