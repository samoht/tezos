...
PROXY_SCRIPT = os.path.join(paths.CONTRACT_PATH, 'baker-proxy.tz')
...
BAKER_ACTIONS = [
    ...
    ('Left { DROP ; NIL baker_operation ; NIL operation ;'
     ...
     'TRANSFER_TOKENS ; CONS ; PAIR }', None),
    # submit proposals
    ('Left { DROP ; NIL baker_operation ;'
     'PUSH (list string) { '
     '"PsCARTHAGazKbHtnKfLzQg3kms52kSRpgnDY982a9oYsSXRLQEb" } ;'
     'SUBMIT_PROPOSALS ; CONS ; NIL operation ; PAIR }', None),
    ...
    ('Left { DROP ; NIL baker_operation ;'
     ...
     'SET_BAKER_PVSS_KEY ; CONS ; NIL operation ; PAIR }', None),
    # set baker owner keys
    ('Right (Pair 1 { '
     '"edpkuBknW28nW72KG6RoHtYW7p12T6GKc7nAbwYX5m8Wd9sDVC9yav" })', None),
]
...
class TestBaker:
    ...
    def test_register_baker(self, client: Client, session):
        ...
        client.register_baker('new-baker-0', 10000, 'bootstrap1',
        ...
        with utils.assert_run_failure(
            ...
            client.register_baker('new-baker-1', 10000, 'bootstrap1',
                                  consensus_key=baker_0_key,
                                  owner_keys=[baker_0_key])
        ...
        with utils.assert_run_failure(
            ...
            client.register_baker('new-baker-2', 10000, 'bootstrap1',
                                  consensus_key=baker_1_key,
                                  owner_keys=[baker_1_key], threshold=2)
    ...
    def test_set_multisig_threshold_and_owner_keys(self, client: Client):
        ...
        with utils.assert_run_failure(expected_failure):
            cmd = ['prepare', 'baker', 'transaction', 'on', baker, 'setting',
                   'threshold', 'to', str(threshold), 'and', 'owner', 'keys',
                   'to', key]
            client.run(cmd)

        with utils.assert_run_failure(expected_failure):
            cmd = ['sign', 'baker', 'transaction', 'on', baker, 'setting',
                   'threshold', 'to', str(threshold), 'and', 'owner', 'keys',
                   'to', key, 'with', 'key', 'baker1_key']
            client.run(cmd)
        ...
        with utils.assert_run_failure(expected_failure):
            ...
            cmd = ['set', 'multisig', 'baker', baker, 'threshold', 'to',
                   str(threshold), 'and', 'owner', 'keys', 'to', key,
                   'on', 'behalf', 'of', 'baker1', 'with', 'signatures',
                   fake_signature]
            client.run(cmd)
    ...
    def test_transfer_to_a_pending_consensus_key_fails(self, client: Client):
        ...
        with utils.assert_run_failure(
            ...
            client.transfer(10000, 'bootstrap1', new_consensus_key,
                            ['--burn-cap', '0.257'])
    ...
    def test_ill_typed_originated_script_with_baker_operation(
        ...
        error_pattern = (r"Type list baker_operation is not compatible with "
                         "type list operation")
        ...
        with utils.assert_run_failure(error_pattern):
            client.originate('noop', 1000, 'bootstrap1', path,
                             ['--burn-cap', '0.295'])
    ...
    def test_get_baker_contract_storage(self, client: Client):
        ...
        keys = [key, new_key_pk]
        pattern = (
            fr'Pair 1\s+ 2\s+{{ \"{keys[0]}\" ;\s+\"{keys[1]}\" }}')
    ...
    def test_originate_proxy(self, client: Client):
        client.originate('proxy', 1000, 'bootstrap1', PROXY_SCRIPT,
                         ['--burn-cap', '0.715'])
        client.bake('baker5', BAKE_ARGS)
    ...
    def test_run_baker_and_baker_proxy_script(
        ...
        client.run_baker_script(storage, param, source=source,
                                entrypoint='main')
    ...
    def test_transfer(self, client: Client):
        ...
        cmd = [
            'from', 'baker', 'contract', source, 'transfer', str(amount), 'to']
        ...
        assert (balance_source - fee_mutez - amount_mutez
                == new_balance_source)
        ...
        client.originate(target, 1000, source, contract,
                         args=['--burn-cap', '0.321'])
        client.bake('baker5', BAKE_ARGS)
        balance_source = client.get_mutez_balance(source)
        balance_target = client.get_mutez_balance(target)
