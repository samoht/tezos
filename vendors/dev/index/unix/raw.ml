...
type t = { mutable close : bool; fd : Unix.file_descr; buf : buffer }
 ...
 *)
let really_write t fd_offset buffer =
  let rec aux fd_offset buffer_offset length =
    let w =
      Syscalls.pwrite ~fd:t.fd ~fd_offset ~buffer ~buffer_offset ~length
    in
...
let really_read t fd_offset length buffer =
  let rec aux fd_offset buffer_offset length =
    let r = Syscalls.pread ~fd:t.fd ~fd_offset ~buffer ~buffer_offset ~length in
